...
<section class="content" >
		...
		<div class="panel">
			<form class="space-y-5" id="myForm" action="{{ route('locations.update',$location->id) }}" method="POST">
				@csrf
				@method('PUT')
			    <div class="flex sm:flex-row flex-col" id="LocationCodeField">
			        ...
			        <input type="text" placeholder="Enter Location Code" class="text-xs form-input flex-1" name="LocationCode" id="LocationCode" value="{{$location->LocationCode}}">
			    </div>
			    <div class="flex sm:flex-row flex-col" id="LocationNameField">
				    ...
				    <input type="text" placeholder="Enter Location Name" class="text-xs form-input flex-1" name="LocationName" id="LocationName" value="{{$location->LocationName}}" >
				</div>
			    <div class="flex sm:flex-row flex-col">
			        <label  class="mb-0 sm:w-1/4 sm:ltr:mr-2 rtl:ml-2">Notes</label>
			       	<textarea class="text-xs flex-1 form-input">{{$location->Notes}}</textarea>
			    </div>
