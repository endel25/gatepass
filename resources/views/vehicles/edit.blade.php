...
<section class="content" >
		<div class="panel">
			<form class="space-y-5" id="myForm" action="{{ route('vehicles.update',$vehicle->id) }}" method="POST">
				@csrf
				 @method('PUT')
			    <div class="flex sm:flex-row flex-col" id="VehicleTypeField">
			        ...
			        <select id="VehicleType" name="VehicleType" class="text-xs flex-1 form-input">
							    ...
							    <option {{$vtypes->FieldValue ==$vehicle->VehicleType?'selected':''}} value="{{$vtypes->FieldValue}}">{{$vtypes->FieldValue}}</option>
							    @endforeach
							</select>
			    </div>
			    <div class="flex sm:flex-row flex-col" id="VehicleNoField">
					    ...
					    <input type="text" placeholder="Enter Vehicle No." class="text-xs form-input flex-1" name="VehicleNo" id="VehicleNo" value="{{$vehicle->VehicleNo}}" oninput="this.value = this.value.toUpperCase()">
			    ...
			    <div class="flex sm:flex-row flex-col" id="TransporterField">
			        ...
			        <select id="TransporterId" name="TransporterId" class="text-xs flex-1 form-input" >
							    ...
							    <option {{$transporters->id == $vehicle->TransporterId ? 'selected':''}} value="{{$transporters->id}}">{{$transporters->TransporterName}}</option>
							    @endforeach
							</select>
			    </div>
			    <div class="flex sm:flex-row flex-col">
			        <label  class="mb-0 sm:w-1/4 sm:ltr:mr-2 rtl:ml-2">Notes</label>
			       	<textarea class="text-xs flex-1 form-input" name="Notes">{{$vehicle->Notes}}</textarea>
			    </div>
