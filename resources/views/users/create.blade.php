...
<section class="content">
  <div class="panel">
    <form class="space-y-2" id="myForm" action="{{ route('users.store') }}" method="POST" autocomplete="off">
      @csrf
      <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
        ...
        <div class="flex-1 max-w-lg space-y-4 sm:space-x-4">
          <div class="flex sm:flex-row flex-col" id="FirstNameField">
            ...
            <input type="text" placeholder="Enter First Name" class="text-xs form-input flex-auto" name="FirstName" id="FirstName" value="{{old('FirstName')}}">
          </div>
          <div class="flex sm:flex-row flex-col" id="LastNameField">
            ...
            <input type="text" placeholder="Enter Last Name" class="text-xs form-input flex-auto" name="LastName" id="LastName" value="{{old('LastName')}}">
          </div>
          <div class="flex sm:flex-row flex-col" id="UserNameField">
             ...
             <input type="text" placeholder="Enter User Name" class="text-xs form-input flex-auto" name="email" id="email" value="{{old('email')}}" autocomplete="off" oninput="this.value = this.value.replace(/\s+/g, '').toLowerCase()">
         </div>
        ...
        <div class="flex-1 max-w-lg space-y-4" style="margin-left: 2.2rem;">
         <div class="flex sm:flex-row flex-col" id="ContactNoField">
            ...
            <input type="text" placeholder="Enter Contact No." class="text-xs form-input flex-auto" x-mask="999-9999999" name="ContactNo" id="ContactNo" value="{{old('ContactNo')}}" autocomplete="off">
         </div>
          <div class="flex sm:flex-row flex-col" id="EmailIdField">
            ...
            <input type="email" placeholder="Enter Email Id" class="text-xs form-input flex-auto" name="EmailId" id="EmailId" value="{{old('EmailId')}}">
          </div>
          <div class="flex sm:flex-row flex-col" id="AddressField">
            ...
            <input type="text" placeholder="Enter Address" class="text-xs form-input flex-auto" name="Address" id="Address" value="{{old('Address')}}">
          </div>
          <div class="flex sm:flex-row flex-col" id="UserNameField">
            ...
            <select id="UserRoleId" name="UserRoleId" class="text-xs flex-1 flex-auto form-input">
               ...
               @foreach($userrole as $userroles)
                  <option {{old('UserRoleId') == $userroles->id ?'selected':''}} value="{{$userroles->id}}">{{$userroles->UserRoleName}}</option>
               @endforeach
            </select>
          </div>
