...
<section class="content-header">
   <div class="mb-5 flex items-center justify-between">
      <h5 class="text-lg font-semibold dark:text-white-light">Transporter Management Stack</h5>
      <a type="button" href="{{ route('transporter.create') }}" class="btn btn-sm btn-l btn-primary rounded-full">Add Transporter</a>
   </div>
</section>
<section>
   <div class="panel">
      <table id="myTable" class="table-hover whitespace-nowrap">
         ...
         <tbody>
            @foreach($transpoter as $transpoters)
            <tr>
               <td>{{$transpoters->TransporterCode}}</td>
               <td>{{$transpoters->TransporterName}}</td>
               <td>
                  <div class="flex items-center  gap-4" >
                     <a href="{{ route('transporter.edit',$transpoters->id) }}" title="EDIT">
                    ...
                    <a onclick="showAlert('{{URL::to('/')}}/transporter/{{ $transpoters->id }}/delete')" title="DELETE">
